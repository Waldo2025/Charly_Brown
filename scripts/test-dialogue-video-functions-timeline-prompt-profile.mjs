...
assert.match(backendResult.prompt, /Interior cinematográfico/);
assert.match(backendResult.prompt, /continente americano/i);
assert.match(backendResult.prompt, /Latinoamérica/i);
assert.match(backendResult.prompt, /Conserva continuidad con la escena anterior en el primer fotograma y luego obedece el nuevo contenido solicitado\./);
assert.match(backendResult.prompt, /La continuidad con la escena anterior es obligatoria solo como punto de arranque del clip;/);
assert.doesNotMatch(backendResult.prompt, /Prioridad máxima:/);

console.log("Backend timeline scene prompt profile regression OK.");
