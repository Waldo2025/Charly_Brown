...
test("scene drag handle ignores Gemini and audio chips", () => {
  ...
  assert.match(source, guardedDragClip);
});

test("scene duration changes compact following clips on the same track", () => {
  assert.match(source, /if \(durationChanged\) \{[\s\S]*?persistCompactedTimelineTrackFromRow\(rowId, \{ render: false \}\);/m);
  assert.match(source, /setGenerationStatus\(`Escena restablecida a \$\{restoredSec\}s`, "is-live"\);[\s\S]*?persistCompactedTimelineTrackFromRow\(rowId, \{ render: false \}\);/m);
});

test("gemini segment drag updates only gemini chip preview instead of rerendering scene clips", () => {
  assert.match(source, /if \(drag\.mode === "gemini-segment-move"\) \{[\s\S]*?syncTimelineGeminiSegmentDragPreview\(getActiveSession\(\)\);/m);
});
