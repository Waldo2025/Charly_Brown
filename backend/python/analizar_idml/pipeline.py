...
FIELD_FORMATIVO_CATALOG = [
    ...
    },
    {
        ...
        "swatchAliases": ["CF ETICA NAT Y SOC"],
    },
    {
        ...
        "swatchAliases": ["CF ROSA HUM A COM"],
    },
    {
        ...
        "swatchAliases": ["CF SABERES TECH y AMBIEN"],
    },
]
...
def _build_swatch_lookup(swatches):
    ...
    for swatch in swatches or []:
        ...
        if name:
            lookup[name] = swatch
            lookup[_normalize_style_name(name)] = swatch
    return lookup


def _select_semantic_story_blocks(stories, limit=120):
    ranked = sorted(
        (entry for entry in (stories or []) if len(str((entry or {}).get("text") or "").strip()) >= 20),
...
def _build_swatch_inventory(swatches, limit=None):
    return [
        {
        ...
        for swatch in ((swatches or []) if limit is None else (swatches or [])[:limit])
    ]
...
def _is_campo_formativo_style(style_name="", alias_index=None):
    normalized = _normalize_style_name(style_name)
    return (
        _has_alias_style(alias_index, "campo_formativo", "paragraph", normalized)
        or "CAMPO FORMATIVO" in normalized
    )
...
def _extract_field_profiles(page, swatch_lookup, alias_index=None):
    ...
    def resolve_field_swatch(field):
        for swatch_name in [field.get("swatchName"), *((field.get("swatchAliases") or []))]:
            normalized = _normalize_style_name(swatch_name)
            swatch = swatch_lookup.get(str(swatch_name or "").strip()) or swatch_lookup.get(normalized) or {}
            if swatch:
                return str(swatch_name or "").strip(), swatch
        return str(field.get("swatchName") or "").strip(), {}

    def field_swatch_names(field):
        return {
            _normalize_style_name(value)
            for value in [field.get("swatchName"), *((field.get("swatchAliases") or []))]
            if _normalize_style_name(value)
        }

    def append_field_profile(field, source="text", matched_swatch_name=""):
        if field["swatchName"] in seen:
            return
        seen.add(field["swatchName"])
        swatch_name, swatch = resolve_field_swatch(field)
        display_swatch_name = str(matched_swatch_name or swatch_name or field["swatchName"]).strip()
        profiles.append({
            "label": field["label"],
            "swatchName": display_swatch_name,
            "canonicalSwatchName": field["swatchName"],
            "cmyk": swatch.get("cmyk") or "",
            "hex": swatch.get("hex") or "",
            "source": source,
        })
    ...
    for item in _iter_page_bucket_items(page, include_master=True):
        style_name = str(item.get("styleName") or "").strip().upper()
        if not _is_campo_formativo_style(style_name, alias_index=alias_index):
            continue
        text = _normalize_story_text(item.get("text"))
        if not text or text.endswith("xxx"):
            continue
        for field in FIELD_FORMATIVO_CATALOG:
            if field["match"] not in text:
                continue
            append_field_profile(field, source="text")
            break
    if profiles:
        return profiles
    ...
    for field in FIELD_FORMATIVO_CATALOG:
        matched_swatch = next((name for name in field_swatch_names(field) if name in used_swatches), "")
        if matched_swatch:
            append_field_profile(field, source="swatch", matched_swatch_name=matched_swatch)
    return profiles
    ...
    for page in page_reports or []:
        ...
        for block_type, entries in (page.get("content") or {}).items():
            for entry in entries or []:
                ...
                if not text:
                    continue
                story_id = str(entry.get("storyId") or "").strip()
                if not story_id:
                ...
                if text not in bucket["textParts"]:
                    bucket["textParts"].append(text)
                if block_type and block_type not in bucket["blockTypes"]:
                    bucket["blockTypes"].append(block_type)
                style_name = entry.get("styleName") or ""
                if style_name and style_name not in bucket["styleNames"]:
                    bucket["styleNames"].append(style_name)
    blocks = []
    for bucket in grouped.values():
        text_parts = [str(part or "").strip() for part in bucket.pop("textParts", []) if str(part or "").strip()]
        if not text_parts:
            continue
        block_types = bucket.pop("blockTypes", [])
        style_names = bucket.pop("styleNames", [])
        bucket["text"] = "\n".join(text_parts)
        bucket["blockType"] = " · ".join(block_types) if block_types else bucket.get("blockType") or ""
        bucket["styleName"] = " · ".join(style_names) if style_names else bucket.get("styleName") or ""
...
def _attach_page_level_findings(page_reports, pagination_rows, spelling_issues, orthotypography_issues, redaction_issues=None):
    ...
    for issue in redaction_issues or []:
        redaction_by_page.setdefault(str(issue.get("pageName") or "").strip(), []).append(issue)
    for page in page_reports or []:
        page_name = str(page.get("pageName") or "").strip()
        ...
        page["redactionIssues"] = redaction_by_page.get(page_name, [])
...
def _extract_linked_asset_mentions(text, allowed_types=None):
    ...
    for raw_type, raw_code in matches:
        ...
        if not _is_probable_linked_asset_code(kind, code):
            continue
...
def _is_probable_linked_asset_code(kind="", code=""):
    import re
    ...
    if normalized_kind == "video":
        return True
    if normalized_kind in {"recortable", "ficha", "anexo"}:
        return bool(re.fullmatch(r"(?:\d+[A-Za-z](?:T\d+)?|[A-Za-z]{1,3}T\d+)", normalized_code, flags=re.IGNORECASE))
    return True
...
def _extract_destination_page_labels(text):
    import re

    source = " ".join(str(text or "").split())
    if not source:
        return []
    match = re.search(r"\b(P[aá]ginas?)\b(.+)$", source, flags=re.IGNORECASE)
    if not match:
        return []
    ...
    if not tail:
        return []
    raw_values = [str(value or "").strip() for value in re.findall(r"\d{1,4}", tail) if str(value or "").strip()]
    if not raw_values:
        return []
    normalized_candidates = []
    if heading.startswith("página"):
        primary_value = raw_values[0]
        normalized_candidates = [primary_value[:3] if len(primary_value) > 3 else primary_value]
    else:
        shorter_values = [value for value in raw_values if len(value) <= 3]
        if shorter_values:
            normalized_candidates = shorter_values
        else:
            normalized_candidates = [value[:3] if len(value) > 3 else value for value in raw_values]
    seen = set()
    labels = []
...
def _resolve_active_analysis_revision(session=None):
    revisions = ((session or {}).get("revisions") or [])
    analysis_context = (session or {}).get("analysisContext") or {}
    analysis_revision_id = str(analysis_context.get("revisionId") or "").strip()
    if analysis_revision_id:
        for revision in revisions:
            if str((revision or {}).get("id") or "").strip() == analysis_revision_id:
                return revision
    current_unidad = _normalize_unidad((((session or {}).get("bibliographicInfo")) or {}).get("unidad") or "")
    current_revision_numero = str((((session or {}).get("bibliographicInfo")) or {}).get("revisionNumero") or "").strip().lower()
    if current_unidad or current_revision_numero:
        for revision in revisions:
            revision_unidad = _normalize_unidad((revision or {}).get("unidad") or "")
            revision_numero = str((revision or {}).get("revisionNumero") or "").strip().lower()
            if revision_unidad == current_unidad and revision_numero == current_revision_numero:
                return revision
    return revisions[0] if revisions else None


def _resolve_active_recortable_role(session=None):
    analysis_context = (session or {}).get("analysisContext") or {}
    explicit_role = str(analysis_context.get("recortableRole") or "").strip().lower()
    if explicit_role in {"source", "destination", "both"}:
        return explicit_role
    active_revision = _resolve_active_analysis_revision(session)
    revision_role = str((active_revision or {}).get("recortableRole") or "").strip().lower()
    if revision_role in {"source", "destination", "both"}:
        return revision_role
    bibliographic_role = str((((session or {}).get("bibliographicInfo")) or {}).get("recortableRole") or "").strip().lower()
    if bibliographic_role in {"source", "destination", "both"}:
        return bibliographic_role
    return "source"


def _origins_match_destination_labels(origins=None, destination_labels=None):
    origin_values = [str(value or "").strip() for value in (origins or []) if str(value or "").strip()]
    destination_values = [str(value or "").strip() for value in (destination_labels or []) if str(value or "").strip()]
    if not origin_values or not destination_values:
        return False
    origin_set = set(origin_values)
    destination_set = set(destination_values)
    if len(destination_set) == 1:
        return next(iter(destination_set)) in origin_set
    return origin_set == destination_set


def _resolve_item_text_with_story_context(item=None, story_preview_index=None):
    direct_text = str((item or {}).get("text") or "").strip()
    story_id = str((item or {}).get("storyId") or "").strip()
    if not story_id:
        return direct_text
    story = (story_preview_index or {}).get(story_id) or {}
    flow_text = " ".join(
        ...
        for token in ((story or {}).get("flowTokens") or [])
        if str((token or {}).get("type") or "").strip() == "text" and str((token or {}).get("text") or "").strip()
    ).strip()
    story_text = " ".join(str((story or {}).get("text") or "").split()).strip()
    if flow_text and (not story_text or len(flow_text) >= len(story_text)):
        story_text = " ".join(flow_text.split()).strip()
    if story_text and (not direct_text or direct_text in story_text) and len(story_text) > len(direct_text):
        return story_text
    return direct_text
...
def _build_external_asset_destination_index(session=None):
    from .recortables import _build_external_asset_destination_index as build_index
    return build_index(session)


def _build_external_asset_origin_index(session=None):
    from .recortables import _build_external_asset_origin_index as build_index
    return build_index(session)


def _build_recortable_checks(page_reports, alias_index=None, session=None, gemini_verifier=None, story_preview_index=None):
    from .recortables import _build_recortable_checks as build_checks
    return build_checks(
        page_reports,
