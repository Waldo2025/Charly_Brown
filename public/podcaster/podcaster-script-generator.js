...
import {
  normalizeVideoTableHeaderKey as normalizeVideoTableHeaderKeyShared,
  normalizeEducationalVideoTableRows as normalizeEducationalVideoTableRowsShared,
  validateDirectVideoTableRows as validateDirectVideoTableRowsShared
} from "./podcaster-video-table-mapping.js";
import {
...
function resolveScriptModelName() {
  ...
  return String(select?.value || "").trim() || "gemini-3.5-flash";
}
...
function compactScriptForPanelConnection(script = {}, session = null, options = {}) {
  const videoMode = options?.videoMode === true
    || script?.videoMode === true
    || normalizeVideoContentType(script?.videoContentType) === "creative"
    || isCurrentModeVideo();
  const compacted = normalizeCreativeVideoScriptForDisplay(script, session, {
    preserveExactRows: true,
    videoMode: videoMode
  });
  if (!videoMode) return compacted;
  return {
    ...compacted,
    rows: normalizeRows(compacted?.rows).map((row, index) => normalizeCreativeRow(row, index, {
      videoPreset: normalizeVideoPreset(compacted?.videoPreset || "creative")
    }))
  };
}
...
function preserveExistingInspectorText(incomingRow = {}, existingRow = null) {
  if (!existingRow || typeof existingRow !== "object") return incomingRow;
  const existingInSceneText = String(existingRow?.inSceneText || "")
    .replace(/\r\n?/g, "\n")
    .trim();
  if (!existingInSceneText) return incomingRow;
  return {
    ...incomingRow,
    inSceneText: existingInSceneText,
    inSceneTextEditedStored: existingRow?.inSceneTextEditedStored === true || Boolean(existingInSceneText)
  };
}
    ...
    ? (raw.rows.length
      ? raw.rows.map((row, index) => {
        ...
        return {
          ...
          headlineText: String(row?.headlineText ?? row?.onScreenText ?? "").replace(/\s+/g, " ").trim(),
          captionText: String(row?.captionText || "").replace(/\s+/g, " ").trim(),
          inSceneText: String(row?.inSceneText || row?.textoPantalla || row?.textoEnPantalla || "").replace(/\s+/g, " ").trim(),
          overlayMode: String(row?.overlayMode || "").trim(),
...
function summarizeOnScreenTextFromVoiceOver(text = "", options = {}) {
  ...
  if (source.length > 48 || words.length < 2 || words.length > 6) return "";
  if (danglingWords.has(String(words[words.length - 1] || "").toLowerCase())) return "";
...
function isWeakOnScreenText(value = "") {
  ...
  const words = text.split(/\s+/).filter(Boolean);
  if (text.length > 48 || words.length < 2 || words.length > 6) return true;
...
async function enhanceEducationalVideoOnScreenTextWithGemini(rows = [], sessionSnapshot = null) {
    ...
    properties: {
      rows: {
        ...
        items: {
          ...
          properties: {
            index: { type: "number" },
            headlineText: { type: "string", nullable: true }
          },
          required: ["index", "headlineText"]
        }
      }
    },
    required: ["rows"]
  ...
  const compactRows = normalizedRows.map((row, index) => ({
    ...
    titularActual: String(sourceRows[index]?.headlineText || "").trim()
  }));
  ...
  const payload = {
    systemInstruction: {
      parts: [{
        text: [
          "Tarea: podcaster_headline_copy_v2.",
          "Eres editor de titulares editoriales para video corto en español latinoamericano.",
          "Devuelve por fila un titular opcional de 2 a 6 palabras y máximo 48 caracteres.",
          "Debe expresar una idea completa, conservar acentos y no copiar ni truncar las primeras palabras del guion.",
          "No dupliques literalmente la descripción de escena, transición ni elemento visual.",
          "Usa null cuando un titular no aporte valor. Responde solo JSON válido."
        ].join(" ")
      }]
    },
    contents: [{
  ...
  const data = await authFetchJson("/api/gemini/generate", {
    method: "POST",
    body: JSON.stringify({
      task: "podcaster_headline_copy_v2",
      taskProfile: "podcaster_headline_copy_v2",
  ...
  (Array.isArray(parsed?.rows) ? parsed.rows : []).forEach((item) => {
    ...
    const candidate = item?.headlineText == null ? "" : normalizeSimpleText(item.headlineText);
    mapped.set(index, isWeakOnScreenText(candidate) ? "" : candidate);
  });
  return normalizedRows.map((row, index) => {
    const sourceRow = sourceRows[index] || {};
    const isManual = sourceRow?.textSource === "manual";
    const headlineText = isManual
      ? normalizeSimpleText(sourceRow?.headlineText || "")
      : (mapped.has(index) ? mapped.get(index) : normalizeSimpleText(sourceRow?.headlineText || ""));
    const captionText = normalizeSimpleText(sourceRow?.script || row?.script || "");
    return {
      ...row,
      headlineText,
      captionText,
      inSceneText: normalizeSimpleText(sourceRow?.inSceneText || ""),
      overlayMode: headlineText ? "both" : "captions",
      textSource: isManual ? "manual" : "generated",
...
function buildOnScreenText(value = "", options = {}) {
  ...
  if (!explicit || looksLikeTransitionOnly(explicit) || isWeakOnScreenText(explicit)) return "";
  return explicit;
}
...
function expandCreativeVideoRowsForTiming(rows = [], options = {}) {
  ...
  sourceRows.forEach((row, rowIndex) => {
    ...
    const sourceHeadlineText = normalizeSimpleText(row?.headlineText || "");
    const sourceCaptionText = normalizeSimpleText(row?.captionText || "");
    const captionFollowsDialogue = sourceCaptionText
      && normalizeComparableCreativeText(sourceCaptionText) === normalizeComparableCreativeText(voiceOverText);
    chunks.forEach((chunk, chunkIndex) => {
      const headlineText = chunkIndex === 0 ? sourceHeadlineText : "";
      const captionText = captionFollowsDialogue
        ? chunk
        : (chunkIndex === 0 ? sourceCaptionText : "");
      const nextRow = {
        ...row,
        id: makeId("row"),
        durationSec: VIDEO_SCENE_MAX_SEC,
        ...
        headlineText,
        captionText,
        inSceneText: chunkIndex === 0 ? normalizeSimpleText(row?.inSceneText || "") : "",
        overlayMode: headlineText && captionText
          ? "both"
          : (headlineText ? "headline" : (captionText ? "captions" : "none")),
        textSource: String(row?.textSource || "generated").trim() || "generated",
        onScreenText: headlineText || captionText,
...
function resolvePodcasterVideoAspectRatio(session = null) {
  const activeSession = session || (() => {
    try { return getActiveSession(); } catch (_) { return null; }
  })();
...
function buildFallbackCreativeSceneDescriptionFromVoiceOver(voiceOver = "", transition = "", index = 0) {
  ...
  const aspectRatio = resolvePodcasterVideoAspectRatio();
  const parts = [
    `Escena cinematográfica en ${aspectRatio} que muestra: ${topic}.`,
...
function buildCreativeOnScreenText(value = "", options = {}) {
  const explicit = normalizeSimpleText(value);
  void options;
  return buildOnScreenText(explicit);
}
...
function buildReelYoutuberVisualNotes(row = {}, index = 0) {
  ...
  const topicClause = topicPreview ? " mientras explica visualmente el concepto principal de la escena" : "";
  const overlayHints = [
    "iconos didácticos animados sin palabras",
    "diagrama explicativo flotante sin etiquetas",
    "formas geométricas grandes sin letras",
    "gráfico sencillo de apoyo sin rótulos",
    "flecha animada sin texto señalando el concepto clave"
  ][index % 5];
...
function validateCreativeVideoScriptOutput(script = {}, options = {}) {
  ...
  rows.forEach((row, index) => {
    ...
    const videoDirective = normalizeSimpleText(row?.videoDirective || "");
    const headlineText = normalizeSimpleText(row?.headlineText || "");
    const inSceneText = normalizeSimpleText(row?.inSceneText || "");
...
function applyAuthoritativeCreativeTableToScript(script = {}, tableScript = null, session = null) {
  ...
  const mergedRows = authoritativeRows.map((tableRow, index) => {
    ...
    const headlineText = normalizeSimpleText(tableRow?.headlineText || "");
    if (!voiceOverText) {
    ...
    const merged = normalizeCreativeRow({
      ...
      inSceneText: normalizeSimpleText(tableRow?.inSceneText || ""),
      overlayMode: headlineText ? "both" : "captions",
      textSource: String(tableRow?.textSource || "manual").trim() || "manual",
      onScreenTextNoSummarize: true,
      visualNotes,
...
function mergeWithPreviousScript(generated = {}, previous = {}, options = {}) {
  ...
  for (let i = 0; i < targetCount; i += 1) {
    ...
    mergedRows.push({
      ...
      ...(prev?.textSource === "manual" ? {
        headlineText: String(prev?.headlineText || "").trim(),
        captionText: String(prev?.captionText || "").trim(),
        inSceneText: String(prev?.inSceneText || "").trim(),
        overlayMode: String(prev?.overlayMode || "none").trim() || "none",
        textSource: "manual",
        onScreenText: String(prev?.onScreenText || "").trim()
      } : {})
    });
  }
...
function buildScriptGenerationResponseSchema(options = {}) {
  ...
  const rowProperties = {
    ...
    inSceneText: { type: "string" },
    overlayMode: { type: "string", enum: ["none", "headline", "captions", "both"] },
    textSource: { type: "string", enum: ["generated", "manual", "migrated"] },
  ...
  const rowRequired = videoMode
    ? ["durationSec", "voiceOverText", "sceneDescription", "transition", "visualNotes", "videoDirective", "scenePrompt", "imagePrompts"]
...
async function generateScriptWithGeminiCore(prompt, sessionSnapshot = null, constraints = null, pipeline = "podcast") {
  ...
  try {
    data = await authFetchJson("/api/gemini/generate", {
      method: "POST",
      body: JSON.stringify({
        ...(videoMode && videoPreset === "creative" ? { taskProfile: "podcaster_creative_video_script_v2" } : {}),
...
function connectScriptSnapshotToPanel(scriptSnapshot = {}, options = {}) {
  ...
  const updatedSession = upsertActiveSession((current) => {
    ...
    const currentRows = normalizeRows(current?.script?.rows);
    const currentRowsById = new Map(currentRows
    ...
    const connectedRows = normalizeRows(nextScriptWithDisfluency.rows).map((row, index) => {
      ...
      const existingRow = (incomingId && currentRowsById.get(incomingId)) || currentRows[index] || null;
      const rowWithPreservedInspectorText = preserveExistingInspectorText(row, existingRow);
      const expectedSpeaker = hosts[index % Math.max(1, hosts.length)] || hosts[0] || "Host A";
      const speaker = resolveSpeakerFromAliases(String(rowWithPreservedInspectorText?.speaker || "").trim(), {
        hosts,
        fallback: expectedSpeaker,
        aliasMap,
        nameMap: maps.nameMap
      });
      if (normalizeVoiceNameSource(rowWithPreservedInspectorText?.voiceNameSource) === "row") {
        return normalizeRowVoiceConfig(rowWithPreservedInspectorText, current, {
          speaker,
          hostVoiceName: maps.voiceMap[speaker],
          voiceName: rowWithPreservedInspectorText?.voiceName,
          voiceNameSource: "row"
        });
      }
      return normalizeRowVoiceConfig({
        ...rowWithPreservedInspectorText,
        voiceName: "",
...
function validateDirectVideoTableRows(rawRows = []) {
  return validateDirectVideoTableRowsShared(rawRows);
}
...
function buildVideoScriptFromUnmodifiedTable(promptText = "", promptHtml = "", sessionSnapshot = null) {
  ...
  const rows = validation.rows.map((row, index) => normalizeCreativeRow({
    ...
    sceneDescription: row.sceneDescription,
    headlineText: row.headlineText || "",
    captionText: row.script,
    inSceneText: row.inSceneText || "",
    overlayMode: row.headlineText ? "both" : "captions",
    textSource: "migrated",
    onScreenTextNoSummarize: true,
    transition: row.transition,
...
function normalizeVideoTableHeaderKey(label = "") {
  return normalizeVideoTableHeaderKeyShared(label);
}

function normalizeEducationalVideoTableRows(rows = []) {
  return normalizeEducationalVideoTableRowsShared(rows);
}

function buildEducationalVideoTableHtml(rows = []) {
  const normalizedRows = normalizeEducationalVideoTableRows(rows.map((row) => [row.time, row.script, row.sceneDescription, row.inSceneText, row.transition, row.visual]));
  if (!normalizedRows.length) return "";
  const body = normalizedRows
    .map((row) => `
      <tr>
        ...
        <td>${escapeHtml(row.inSceneText || "")}</td>
...
function buildEducationalVideoTableMarkdown(rows = []) {
  ...
  const body = normalizedRows.map((row) => (
    `| ${toMarkdownTableCell(row.time)} | ${toMarkdownTableCell(row.script)} | ${toMarkdownTableCell(row.sceneDescription)} | ${toMarkdownTableCell(row.inSceneText || "")} | ${toMarkdownTableCell(row.transition)} | ${toMarkdownTableCell(row.visual)} |`
  )).join("\n");
  return `${head}\n${body}`;
}
...
function normalizeOnScreenTextStrict(value = "", options = {}) {
  void options;
  return buildOnScreenText(value);
}
...
function validateEducationalVideoCanonicalRow(row = {}) {
  ...
  if (headlineText && isWeakOnScreenText(headlineText)) issues.push("headline_invalid");
  return {
    ...
    onScreenWords: countWords(headlineText)
  };
}
...
function toEducationalVideoCanonicalRow(raw = {}, index = 0) {
  ...
  const headlineText = normalizeOnScreenTextStrict(raw?.headlineText || "", {
    voiceOver: script,
    sceneDescription,
    visual
  });
  const inSceneText = normalizeSimpleText(raw?.inSceneText || raw?.textoPantalla || raw?.textoEnPantalla || "");
  ...
  const row = {
    ...
    inSceneText,
    overlayMode: headlineText ? "both" : "captions",
    textSource: String(raw?.textSource || "migrated").trim() || "migrated",
    onScreenTextNoSummarize: true,
    transition,
...
function repairEducationalVideoCanonicalRowLocal(row = {}, index = 0) {
  ...
  repaired.headlineText = normalizeOnScreenTextStrict(row?.headlineText ?? "", {
    voiceOver: repaired.script,
    sceneDescription: repaired.sceneDescription,
    visual: repaired.visual
  });
  repaired.captionText = repaired.script;
  repaired.overlayMode = repaired.headlineText ? "both" : "captions";
  repaired.onScreenText = repaired.headlineText ? `${repaired.headlineText}\n${repaired.captionText}` : repaired.captionText;
  repaired.onScreenTextNoSummarize = true;
  ...
  if (!repaired.validation.isValid) {
    ...
    repaired.headlineText = normalizeOnScreenTextStrict(repaired.headlineText || "", {
      voiceOver: repaired.script,
      sceneDescription: repaired.sceneDescription,
      visual: repaired.visual
    });
...
function canonicalRowsToEducationalVideoTableRows(rows = []) {
  return (Array.isArray(rows) ? rows : []).map((row, index) => {
    const script = normalizeSimpleText(row?.script || row?.voiceOverText || row?.text || "");
    const headlineText = normalizeSimpleText(row?.headlineText || "");
    return {
      ...
      headlineText,
      captionText: script,
      inSceneText: normalizeSimpleText(row?.inSceneText || ""),
      overlayMode: headlineText ? "both" : "captions",
...
function mergeEducationalVideoRowsWithLocalPriority(localRows = [], geminiRows = []) {
  ...
  return geminiRows.map((row, index) => {
    ...
    const script = String(localRow.script || row.script || "").trim() || "Definir voz en off.";
    const headlineText = String(localRow.headlineText || row.headlineText || "").trim();
    return {
      ...
      headlineText,
      captionText: script,
      inSceneText: String(localRow.inSceneText || row.inSceneText || "").trim(),
      overlayMode: headlineText ? "both" : "captions",
      textSource: String(localRow.textSource || row.textSource || "migrated").trim() || "migrated",
      onScreenTextNoSummarize: true,
  ...
  sourceRows.forEach((row) => {
    ...
    chunks.forEach((chunk, index) => {
      ...
      expanded.push({
        ...
        voiceOverText: chunk,
        text: chunk,
        headlineText: index === 0 ? String(row?.headlineText || "").trim() : "",
        inSceneText: index === 0 ? String(row?.inSceneText || row?.textoPantalla || row?.textoEnPantalla || "").trim() : "",
        captionText: chunk,
        overlayMode: index === 0 && String(row?.headlineText || "").trim() ? "both" : "captions",
        onScreenTextNoSummarize: true,
...
async function repairEducationalVideoCanonicalRowsWithGemini(canonicalRows = [], sessionSnapshot = null) {
  ...
  const compactRows = invalidRows.map((item) => ({
    ...
    headlineText: item.row.headlineText || "",
...
No modifiques titulares, captions ni texto natural dentro de escena.
...
async function composeEducationalVideoTable(input = {}, sessionSnapshot = null, options = {}) {
  ...
  canonicalRows = canonicalRows.map((row, index) => {
    ...
    return repairEducationalVideoCanonicalRowLocal({
      ...
      headlineText: normalizeOnScreenTextStrict(row?.headlineText || "")
    }, index);
  });
...
async function composeVideoScriptFromUserInput(promptText = "", promptHtml = "", sessionSnapshot = null) {
  ...
  const rows = canonicalRows.map((row, index) => normalizeCreativeRow({
    ...
    sceneDescription: row?.sceneDescription || "",
    headlineText: row?.headlineText || "",
    captionText: row?.script || row?.voiceOverText || row?.text || "",
    inSceneText: row?.inSceneText || "",
    overlayMode: String(row?.headlineText || "").trim() ? "both" : "captions",
    textSource: row?.textSource || "migrated",
    onScreenTextNoSummarize: true,
    transition: row?.transition || "",
...
registerPodcasterScriptGeneratorApi({
  ...
  buildOnScreenText,
  enhanceEducationalVideoOnScreenTextWithGemini,
